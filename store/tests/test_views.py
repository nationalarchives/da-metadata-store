...
class RecordsViewTest(AuthenticatedViewTest):
    ...
    def test_records_download_returns_correct_json(self):
        self.client.force_login(self.user)
        response = self.client.get(f"/records/{self.metadata.id}/download")
        self.assertEqual(response.json(), {"title": "Test Document"})
